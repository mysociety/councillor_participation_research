# Modified version of jtools plot_coefs function
# change some visuals, colour by significance
...
find_S3_class <- jtools:::find_S3_class
...
plot_summs <- function(..., ci_level = .95, model.names = NULL, coefs = NULL,
  ...
  args <- as.list(c(the_summs, coef_groups = groups, ci_level = ci_level,
...
plot_coefs <- function(..., ci_level = .95, inner_ci_level = NULL,
                       ...
                       legend.title = "Model", coef_groups,
  ...
  if (inherits(dots[[1]], 'list')) {
    ...
    if (all(nchar(names(dots))) > 0) {
      ...
      models <- !is.na(sapply(dots, function(x) {
        out <- jtools:::find_S3_class("tidy", x, package = "generics")
  ...
  tidies$conf.good <- "No effect"
  tidies$conf.good[tidies$conf.low >0] <- "Positive effect"
  tidies$conf.good[tidies$conf.high < 0] <- "Negative effect"
  
  tidies$model <- tidies$conf.good
  ...
  p <- ggplot(data = tidies, aes(y = term, x = estimate, xmin = conf.low,
                                 xmax = conf.high, colour = model, group=model))
  ...
  p <- p + geom_vline(xintercept = 1 - !exp, linetype = 2, size = .25)
