...
for (v in options) {
  ...
  test_results <- wtd.t.test.with.paired.bounds(df$v1_as_scale, df$dexercise, df$weighted)
...
g <- ggplot(df, aes(
  ...
  xlab("") +
  #xlim(-0.4, 1.2) +
  ylab("Awareness scores of policy areas with difference (95% paired interval)") +
